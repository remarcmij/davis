...
export class ALU {
  ...
  add(op1: number, op2: number, previousCarry: number = 0): number {
    ...
    let result = 0;
    const carry: number[] = [previousCarry, 0];

    for (let i = 0; i < 32; i++) {
      carry[0] = carry[1];
      const value: number = (op1 & 1) + (op2 & 1) + carry[0];
      op1 >>= 1;
      op2 >>= 1;
  ...
  imultiply(op1: number, op2: number): { lowerHalf: number, upperHalf: number } {
    ...
    const result: number = op1 * op2;
    const lowerHalf: number = result & 0xFFFFFFFF;
    const upperHalf = result / Math.pow(2, 32);
