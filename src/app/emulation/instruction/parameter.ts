...
export class MemoryParameter extends Parameter {
  ...
  fetchData(cpu: CPU): MemoryView {
    const baseReg: MemoryView = cpu.getRegisterByIndex(this.baseReg);
    const indexReg: MemoryView = cpu.getRegisterByIndex(this.indexer.indexReg);

    return cpu.derefAddress(
      cpu.calculateEffectiveAddressFromRegister(baseReg, indexReg, this.indexer.multiplier, this.indexer.constant),
      this.size);
  }
}
